...
bool checkforComment(string line){
  if(line[0]=='.'){
    return true;
  }
  return false;
...
void readFirstNonWhiteSpace(string line,int& index,bool& status,string& data,bool readTillEnd=false){
  ...
  while(index<line.length()&&!checkforSpace(line[index])){//If no whitespace then data
    data += line[index];
    index++;
  }
  ...
  while(index<line.length()&&checkforSpace(line[index])){//Increase index to pass all whitespace
    index++;
  }
}
...
void readByteOperand(string line,int& index,bool& status,string& data){
  ...
  if(line[index]=='C'){
    ...
    while(index<line.length()&&!checkforSpace(line[index])){//In no whitespace then data
      data += line[index];
      index++;
    }
  }
...
char fetchFlagFormat(string data){
